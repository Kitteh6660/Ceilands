...
public class SmallFloatingIslandFeature extends Feature<BlockStateConfiguration>
{
		...
		for (int k = -i; k < i; ++k) {
			for (int l = -i; l < i; ++l) {
				...
				if (d0 < 0.0D) {
					...
					if (isIslandState(blockstate) || blockstate.is(Blocks.SNOW_BLOCK)) {
						if (p_66040_) {
	...
	private double signedDistanceEllipse(int p_66023_, int p_66024_, BlockPos p_66025_, int p_66026_, int p_66027_, double p_66028_) {
		return Math.pow(((double) (p_66023_ - p_66025_.getX()) * Math.cos(p_66028_) - (double) (p_66024_ - p_66025_.getZ()) * Math.sin(p_66028_)) / (double) p_66026_, 2.0D)
				+ Math.pow(((double) (p_66023_ - p_66025_.getX()) * Math.sin(p_66028_) + (double) (p_66024_ - p_66025_.getZ()) * Math.cos(p_66028_)) / (double) p_66027_, 2.0D) - 1.0D;
	}
	...
	private int heightDependentRadiusEllipse(int p_66110_, int p_66111_, int p_66112_) {
		float f = 1.0F;
		float f1 = (f - (float) Math.pow((double) p_66110_, 2.0D) / ((float) p_66111_ * f)) * (float) p_66112_;
		return Mth.ceil(f1 / 2.0F);
	}
		...
		for (int j = -i; j <= i; ++j) {
			for (int k = -i; k <= i; ++k) {
				for (int l = 0; l <= p_66055_; ++l) {
					...
					if (isIslandState(blockstate) || blockstate.is(Blocks.SNOW)) {
						...
						} else if (isIslandState(blockstate)) {
