	...
	public void setupDataPacks(final AddPackFindersEvent event) {
		...
		if (event.getPackType() == PackType.SERVER_DATA) {
			for (Quartet<String, Boolean, String, String> packToCheck : packs) {
                ...
                try (var pack = new PathPackResources(ModList.get().getModFileById(Ceilands.MOD_ID).getFile().getFileName() + ":" + resourcePath, resourcePath)) {
					var metadataSection = pack.getMetadataSection(PackMetadataSection.SERIALIZER);
					if (metadataSection != null) {
					    event.addRepositorySource((packConsumer, packConstructor) -> packConsumer.accept(packConstructor.create("builtin/ceilands-" + packToCheck.getA(), Component.literal(packToCheck.getD()), false, () -> pack, metadataSection, Pack.Position.BOTTOM, PackSource.BUILT_IN, false)));
					}
				}
